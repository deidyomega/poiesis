...
class Supervisor:
    def __init__(self, env: GlitchEnv, app_target: str = "glitch_core.asgi:app") -> None:
        self.env = env
...
async def run_supervisor(env: GlitchEnv | None = None) -> None:
    sup = Supervisor(env or GlitchEnv())
    try:
        await sup.run()
    finally:
        await sup._stop_app()
