...
- **Dev runtime:** `screen` (or tmux) + `glitch start --app-only` (no supervisor). Edit on
...
2. **#general → chat-only for the dev phase** — drop its Write/Edit/Bash/request_deploy so an
   errant edit can't break the live app while there's no supervisor. Re-enable when self-mod
   goes live.
3. **`--reload` dev runner** — `glitch start --app-only --reload` so Python edits are also
   just-refresh.
...
- **Model auth:** the `claude` CLI provides it — `claude login` (subscription, cheaper) or a
  raw `ANTHROPIC_API_KEY`. Don't set the key if you want the subscription. Glitch doesn't
  manage auth (the old `GLITCH_ANTHROPIC_API_KEY` bridge was removed).
...
- **Storage split:** tasks = markdown file (`~/.glitch/pm/task.md`, outside the repo → safe
  from rollback); memories = SQLite rows; souls = git-tracked files in `souls/`.
- **Timezone:** set `GLITCH_TZ` (Matt = `America/Phoenix`, MST, no DST) so the agent's clock
